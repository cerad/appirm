...
/* ============================================================
 * ***5 Replace obsolete DBI with PDO based version
 */
class DBI
{
    protected $conn;
    ...
    {
        $dbName = $dbName ? $dbName : $config['name'];
        ...
        $this->conn = new \PDO($dsn,$config['user'],$config['pass'],array(
    ...
    function fetchRow($sql,$params = array())
    {
        ...
        $rows = $stmt->fetchAll();
        return (count($rows) == 1) ? new DataRow($rows[0]) : null;
    }
}
